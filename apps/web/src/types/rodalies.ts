...
/**
 * Line proximity configuration for rendering merged lines
 * Generated by scripts/analyzeLineProximity.ts
 */
export interface LineProximitySegment {
  lines: string[]; // Line IDs that overlap in this segment
  coordinateRanges: Record<string, { start: number; end: number }>; // Coordinate indices for each line
  center: LngLat; // Approximate center of segment
  offsetPattern: number[]; // Pixel offsets for each line
}

export interface LineProximityConfig {
  threshold: number; // Distance threshold used (meters)
  minSegmentLength: number; // Minimum segment length used
  generatedAt: string; // ISO timestamp
  segments: LineProximitySegment[];
}
